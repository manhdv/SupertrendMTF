...
namespace cAlgo.Indicators
{
    ...
    public class SupertrendMTF : Indicator
    {
        ...
        public override void Calculate(int index)
        {
            ...
            if (customBars.ClosePrices[customIndex] > upBuffer[index - 1])
            {
                trend[index] = 1;
            }
            else if (customBars.ClosePrices[customIndex] < downBuffer[index - 1])
            {
                trend[index] = -1;
            }
            else
            ...
            if (trend[index] < 0 && upBuffer[index] > upBuffer[index - 1])
            {
                upBuffer[index] = upBuffer[index - 1];
            }

            if (trend[index] > 0 && downBuffer[index] < downBuffer[index - 1])
            {
